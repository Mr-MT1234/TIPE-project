...
class Agent:
    def __init__(self, device, stateShape, actionDim, discount, tau, memoryCap, batchSize,
        ...
        self.stateDim = np.prod(stateShape)
        ...
        self.memory = ReplayMemory (memoryCap, (self.stateDim,), actionDim, batchSize)
        self.actor  = ActorNetwork (device, (self.stateDim,), actionDim, *layersActor, outBounds, actorPath)
        self.critic = CriticNetwork(device, (self.stateDim,), actionDim, *layersCritic, criticPath) 
        
        self.actorTarget  = ActorNetwork (device, (self.stateDim,), actionDim, *layersActor, outBounds, actorTargetPath)
        self.criticTarget = CriticNetwork(device, (self.stateDim,), actionDim, *layersCritic, criticTargetPath)
...
class ConvAgent:
    ...
    def learn(self):
        ...
        states, actions, nextStates, rewards, finals = self.memory.sample()
        #states = states.reshape((self.batchSize,1,*self.stateShape))
        #nextStates = nextStates.reshape((self.batchSize,1,*self.stateShape))
        states, actions, nextStates, rewards, finals = (
            tr.tensor(states, device=self.device),
            tr.tensor(actions, device=self.device),
            tr.tensor(nextStates, device=self.device),
    ...
    def decay(self):
        self.actorOptimizer.param_groups[0]['lr'] *= self.learningDecay
        self.criticOptimizer.param_groups[0]['lr'] *= self.learningDecay
        self.noise.sigma *= self.noiseDecay