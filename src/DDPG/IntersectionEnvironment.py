...
class IntersectionEnv(gym.Env):
    def __init__(self,  
        ...
        self.observation_space = gym.spaces.Box(
            low=np.array([[[0,0]]*OBSERVATION_RES]*5),
            high=np.array([[[200, maxSpeed]]*OBSERVATION_RES]*5),
            dtype=np.float32
        )
    ...
    def step(self, action):
        if np.random.random() < self.spawnRate*self.timeStep:
            if np.random.random() < 0.5:
                if not self.rightArm or self.rightArm[-1].position > 5:
                    self.rightArm.append(Vehicle(history=[], direction=RIGHT_DIRECT))
            else:
                if not self.leftArm or self.leftArm[-1].position > 5:
                    self.leftArm.append(Vehicle(history=[], direction=LEFT_DIRECT))
        ...
        for road in [self.body, self.leftReturn, self.rightReturn]:
            if road:
                v = road[0]
                if v.visualID == 1:
                    ...
                    i = v.visualID - 2
                    v.acceleration = calcSteadyAccel(v.velocity, action[2*i], leader.velocity,self.returnLength + 8 - v.position, self.maxAccel, self.timeStep)
        ...
        return self.getState(), reward, self.currentIter >= self.maxIter, False, {}
    ...
    def reset(self):
        ...
        for i in range(2,self.autoCount+2):
            v = Vehicle(history=[], visualID=i, direction=0)
            r = np.random.random()
            if r < 0.5:
                v.direction = RIGHT_DIRECT
                v.position = sL
    ...
    def getState(self):
        state = np.zeros((5,OBSERVATION_RES,2), dtype=np.float32)
        for i, (road, length, _) in enumerate(self.roads):
            for v in road:
                t = int(v.position / length)
                t = min(t, OBSERVATION_RES-1)
                state[i,t] = (v.visualID, v.velocity)
        return state